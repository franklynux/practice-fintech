...
resource "aws_vpc" "eu-west-1-vpc" {
...
resource "aws_cloudwatch_log_group" "eu_west_1_vpc_flow_logs" {
  name              = "/aws/vpc/${var.vpc_name}-flow-logs"
  retention_in_days = var.retention_in_days
}

# IAM Role for VPC Flow Logs
resource "aws_iam_role" "eu_west_1_vpc_flow_logs_role" {
...
resource "aws_iam_role_policy" "eu_west_1_vpc_flow_logs_policy" {
  name = "${var.vpc_name}-flow-logs-policy"
  role = aws_iam_role.eu_west_1_vpc_flow_logs_role.id
...
resource "aws_cloudwatch_log_data_protection_policy" "pci_data_protection" {
  log_group_name = aws_cloudwatch_log_group.eu_west_1_vpc_flow_logs.name
...
resource "aws_flow_log" "eu_west_1_vpc_flow_log" {
  vpc_id               = aws_vpc.eu-west-1-vpc.id
  traffic_type         = "ALL"
  iam_role_arn         = aws_iam_role.eu_west_1_vpc_flow_logs_role.arn
  log_destination      = aws_cloudwatch_log_group.eu_west_1_vpc_flow_logs.arn
